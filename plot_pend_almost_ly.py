...
ly_dir = os.path.join(curr_dir, "data", "Pendulum-v1", "ly", "seed_1")
...
agent = LYAgent(input_dims=env.observation_space.shape[0], n_actions=env.action_space.shape[0], max_action=env.action_space.high, 
                        dt=env.unwrapped.dt, equilibrium_state=torch.tensor([np.array([np.cos(0), np.sin(0), 0])], dtype=torch.float).to(device),
                        update_freq=1024, save_dir=ly_dir)
...
def plot3D(agent):
    with torch.no_grad():
        ...
        equilibrium_lyapunov = agent.lyapunov.forward(eq_state)
        ...
        for i, theta in enumerate(theta_arr):
            for j, theta_dot in enumerate(theta_dot_arr):
                ...
                action = agent.actor.forward(torch.tensor([state], dtype=torch.float).to(agent.actor.device))
    ...
    with torch.no_grad():
        ...
        for i, theta in enumerate(theta_arr):
            for j, theta_dot in enumerate(theta_dot_arr):
                ...
                lyapunov_val = agent.lyapunov(torch.tensor([state], dtype=torch.float).to(agent.actor.device))
                next_ly_val = agent.lyapunov(torch.tensor([obs], dtype=torch.float).to(agent.actor.device))

                decrease_vals[j,i] = (next_ly_val - lyapunov_val).item()

                lyapunov_vals[j, i] += lyapunov_val.item()

                # print(lyapunov_val.item())
        ...
        positive_indices = decrease_vals > 0
        ...
        ax.set_xlabel('Theta')
        ax.set_ylabel('Theta Dot')
        ax.set_title('LY Lyapunov Function (Contour Lines Only)')
