...
mu_list = [0.0, 0.1, 0.5, 0.05, 0.15, 0.25, 0.75]
...
for mu in mu_list:
    ...
    for i in range(1, 11):
        rew.append(np.load(os.path.join(curr_dir, "data", "desc", "InvertedDoublePendulum-v5", "mu_"+str(mu), "seed_" + str(i+10), "reward_arr.npy")))
        episode_steps= np.load(os.path.join(curr_dir, "data", "desc", "InvertedDoublePendulum-v5", "mu_"+str(mu), "seed_" + str(i+10), "step_arr.npy"))
...
plt.xlabel("Steps")
plt.ylabel("Reward")
plt.title("Inverted Double Pendulum Finite Descent")
plt.legend()
plt.grid(True)
