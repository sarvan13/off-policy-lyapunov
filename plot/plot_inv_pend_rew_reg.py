...
mu_list = [0.01, 0.001, 0.0001, 0.005, 0.0005]
...
for mu in mu_list:
    ...
    for i in range(1, 11):
        rew.append(np.load(os.path.join(curr_dir, "data", "reg", "InvertedDoublePendulum-v5", "beta1", "mu_"+str(mu), "seed_" + str(i+10), "reward_arr.npy")))
        episode_steps= np.load(os.path.join(curr_dir, "data", "reg", "InvertedDoublePendulum-v5", "beta1", "mu_"+str(mu), "seed_" + str(i+10), "step_arr.npy"))
...
plt.xlabel("Steps")
plt.ylabel("Reward")
plt.title("Inverted Double Pendulum Regularized")
plt.legend()
plt.grid(True)
