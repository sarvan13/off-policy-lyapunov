...
# mu_list = [0.0, 0.1, 0.01, 0.5, 0.05, 0.75, 0.85, 1.0, 1.1, 1.25]
mu_list = [0.0, 0.1, 0.5, 0.75, 1.0]
...
for mu in mu_list:
    ...
    for i in range(1, 11):
        if i == 8:
            continue
        rew.append(np.load(os.path.join(curr_dir, "data", "desc", "Quadrotor", "mu_"+str(mu), "seed_" + str(i), "returns.npy")))
        steps.append(np.load(os.path.join(curr_dir, "data", "desc", "Quadrotor", "mu_"+str(mu), "seed_" + str(i), "steps.npy")))
...
plt.xlabel("Steps")
plt.ylabel("Reward")
plt.title("Quadrotor Min Descent")
plt.legend()
plt.grid(True)
