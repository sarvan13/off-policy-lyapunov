...
if __name__ == "__main__":
    ...
    obs_ =  torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_next_obs = obs_.reshape((-1,) + envs.single_observation_space.shape)
        ...
        b_next_actions, _, _ ,_ = agent.get_action_and_value(b_next_obs)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_advantages = (1 - args.lyapunov_weight) * b_advantages[mb_inds] + args.lyapunov_weight * torch.min(torch.tensor(0), -(lyapunov.forward(b_next_obs[mb_inds], b_next_actions[mb_inds]) - lyapunov.forward(b_obs[mb_inds], b_actions[mb_inds])) / dt + 0.1).detach()
                if args.norm_adv:
